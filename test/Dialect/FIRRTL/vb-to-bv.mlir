...
firrtl.circuit "Test" {
  ...
  firrtl.module @TestAggregateConstants() {
    ...
    // CHECK{LITERAL}: firrtl.aggregateconstant [[1, 3], [2, 4]] : !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<5>, 2>>
    firrtl.aggregateconstant [[1, 2], [3, 4]] : !firrtl.const.vector<bundle<a: uint<8>, b: uint<5>>, 2>
    // CHECK{LITERAL}: firrtl.aggregateconstant [[[1, 3], [2, 4]], [[5, 7], [6, 8]]] : !firrtl.const.bundle<a: bundle<c: vector<uint<8>, 2>, d: vector<uint<5>, 2>>, b: bundle<e: vector<uint<8>, 2>, f: vector<uint<5>, 2>>>
    ...
    // CHECK{LITERAL}: firrtl.aggregateconstant [[[1, 3], [5, 7], [9, 11]], [[2, 4], [6, 8], [10, 12]]] : !firrtl.const.bundle<a: vector<vector<uint<8>, 2>, 3>, b: vector<vector<uint<8>, 2>, 3>>
  ...
  firrtl.module @TestVectorCreate() {
    ...
    // CHECK: %2 = firrtl.bundlecreate %c1_ui8 : (!firrtl.const.uint<8>) -> !firrtl.const.bundle<a: uint<8>>
    %b0 = firrtl.bundlecreate %c0 : (!firrtl.const.uint<8>) -> !firrtl.const.bundle<a: uint<8>>

    // CHECK: %3 = firrtl.subfield %2[a] : !firrtl.const.bundle<a: uint<8>>
    // CHECK: %4 = firrtl.vectorcreate %3 : (!firrtl.const.uint<8>) -> !firrtl.const.vector<uint<8>, 1>
    // CHECK: %5 = firrtl.bundlecreate %4 : (!firrtl.const.vector<uint<8>, 1>) -> !firrtl.const.bundle<a: vector<uint<8>, 1>>
    %v2 = firrtl.vectorcreate %b0 : (!firrtl.const.bundle<a: uint<8>>) -> !firrtl.const.vector<bundle<a: uint<8>>, 1>

    // CHECK: %6 = firrtl.bundlecreate %1 : (!firrtl.const.vector<uint<8>, 2>) -> !firrtl.const.bundle<a: vector<uint<8>, 2>>
    %b1 = firrtl.bundlecreate %v1 : (!firrtl.const.vector<uint<8>, 2>) -> !firrtl.const.bundle<a: vector<uint<8>, 2>>

    // CHECK: %7 = firrtl.subfield %6[a] : !firrtl.const.bundle<a: vector<uint<8>, 2>>
    // CHECK: %8 = firrtl.vectorcreate %7 : (!firrtl.const.vector<uint<8>, 2>) -> !firrtl.const.vector<vector<uint<8>, 2>, 1>
    // CHECK: %9 = firrtl.bundlecreate %8 : (!firrtl.const.vector<vector<uint<8>, 2>, 1>) -> !firrtl.const.bundle<a: vector<vector<uint<8>, 2>, 1>>
  ...
  firrtl.module @TestVBAggregate() {
    ...
    // CHECK: %6 = firrtl.vectorcreate %2, %5 : (!firrtl.const.uint<8>, !firrtl.const.uint<8>) -> !firrtl.const.vector<uint<8>, 2>
    // CHECK: %7 = firrtl.vectorcreate %1, %4 : (!firrtl.const.uint<5>, !firrtl.const.uint<5>) -> !firrtl.const.vector<uint<5>, 2>
    // CHECK: %8 = firrtl.bundlecreate %6, %7 : (!firrtl.const.vector<uint<8>, 2>, !firrtl.const.vector<uint<5>, 2>) -> !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<5>, 2>>
  ...
  firrtl.module @TestVVBAggregate() {
    // CHECK{LITERAL}: %0 = firrtl.aggregateconstant [[1, 3], [2, 4]] : !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<5>, 2>>
    // CHECK: %1 = firrtl.subfield %0[b] : !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<5>, 2>>
    // CHECK: %2 = firrtl.subfield %0[a] : !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<5>, 2>>
    %0 = firrtl.aggregateconstant [[1, 2], [3, 4]] : !firrtl.const.vector<bundle<a: uint<8>, b: uint<5>>, 2>
    
    // CHECK{LITERAL}: %3 = firrtl.aggregateconstant [[5, 7], [6, 8]] : !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<5>, 2>>
    // CHECK: %4 = firrtl.subfield %3[b] : !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<5>, 2>>
    // CHECK: %5 = firrtl.subfield %3[a] : !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<5>, 2>>
    %1 = firrtl.aggregateconstant [[5, 6], [7, 8]] : !firrtl.const.vector<bundle<a: uint<8>, b: uint<5>>, 2>

    // CHECK: %6 = firrtl.vectorcreate %2, %5 : (!firrtl.const.vector<uint<8>, 2>, !firrtl.const.vector<uint<8>, 2>) -> !firrtl.const.vector<vector<uint<8>, 2>, 2>
    // CHECK: %7 = firrtl.vectorcreate %1, %4 : (!firrtl.const.vector<uint<5>, 2>, !firrtl.const.vector<uint<5>, 2>) -> !firrtl.const.vector<vector<uint<5>, 2>, 2>
    // CHECK: %8 = firrtl.bundlecreate %6, %7 : (!firrtl.const.vector<vector<uint<8>, 2>, 2>, !firrtl.const.vector<vector<uint<5>, 2>, 2>) -> !firrtl.const.bundle<a: vector<vector<uint<8>, 2>, 2>, b: vector<vector<uint<5>, 2>, 2>>
  ...
  firrtl.module @TestRegReset(in %clock: !firrtl.clock) {
    // CHECK{LITERAL}: %0 = firrtl.aggregateconstant [[1, 3], [2, 4]] : !firrtl.const.bundle<a: vector<uint<4>, 2>, b: vector<uint<8>, 2>>
    ...
    // CHECK: %r = firrtl.regreset %clock, %c0_ui1, %0 : !firrtl.clock, !firrtl.const.uint<1>, !firrtl.const.bundle<a: vector<uint<4>, 2>, b: vector<uint<8>, 2>>, !firrtl.bundle<a: vector<uint<4>, 2>, b: vector<uint<8>, 2>>
  ...
  firrtl.module @TestRegResetMaterializedFromExplodedBundle(in %clock: !firrtl.clock) {
    ...
    // CHECK: %1 = firrtl.subfield %0[b] : !firrtl.const.bundle<a: vector<uint<4>, 2>, b: vector<uint<8>, 2>>
    // CHECK: %2 = firrtl.subindex %1[0] : !firrtl.const.vector<uint<8>, 2>
    // CHECK: %3 = firrtl.subfield %0[a] : !firrtl.const.bundle<a: vector<uint<4>, 2>, b: vector<uint<8>, 2>>
    // CHECK: %4 = firrtl.subindex %3[0] : !firrtl.const.vector<uint<4>, 2>
    // CHECK: %5 = firrtl.bundlecreate %4, %2 : (!firrtl.const.uint<4>, !firrtl.const.uint<8>) -> !firrtl.const.bundle<a: uint<4>, b: uint<8>>
    %rval = firrtl.subindex %storage[0] : !firrtl.const.vector<bundle<a: uint<4>, b: uint<8>>, 2>
    // CHECK: %c0_ui1 = firrtl.constant 0 : !firrtl.const.uint<1>
    %rsig = firrtl.constant 0 : !firrtl.const.uint<1>
    // CHECK: %r = firrtl.regreset %clock, %c0_ui1, %5 : !firrtl.clock, !firrtl.const.uint<1>, !firrtl.const.bundle<a: uint<4>, b: uint<8>>, !firrtl.bundle<a: uint<4>, b: uint<8>>
  ...
  firrtl.module @TestRegResetMaterializedFromDeepExplodedBundle(in %clock: !firrtl.clock) {
    // CHECK{LITERAL}: %0 = firrtl.aggregateconstant [[1, 4], [[2, 3], [5, 6]]] : !firrtl.const.bundle<a: vector<uint<4>, 2>, b: bundle<c: vector<uint<8>, 2>, d: vector<uint<16>, 2>>>
    // CHECK: %1 = firrtl.subfield %0[b] : !firrtl.const.bundle<a: vector<uint<4>, 2>, b: bundle<c: vector<uint<8>, 2>, d: vector<uint<16>, 2>>>
    // CHECK: %2 = firrtl.subfield %1[d] : !firrtl.const.bundle<c: vector<uint<8>, 2>, d: vector<uint<16>, 2>>
    // CHECK: %3 = firrtl.subindex %2[1] : !firrtl.const.vector<uint<16>, 2>
    // CHECK: %4 = firrtl.subfield %1[c] : !firrtl.const.bundle<c: vector<uint<8>, 2>, d: vector<uint<16>, 2>>
    // CHECK: %5 = firrtl.subindex %4[1] : !firrtl.const.vector<uint<8>, 2>
    // CHECK: %6 = firrtl.subfield %0[a] : !firrtl.const.bundle<a: vector<uint<4>, 2>, b: bundle<c: vector<uint<8>, 2>, d: vector<uint<16>, 2>>>
    // CHECK: %7 = firrtl.subindex %6[1] : !firrtl.const.vector<uint<4>, 2>
    // CHECK: %8 = firrtl.bundlecreate %5, %3 : (!firrtl.const.uint<8>, !firrtl.const.uint<16>) -> !firrtl.const.bundle<c: uint<8>, d: uint<16>>
    // CHECK: %9 = firrtl.bundlecreate %7, %8 : (!firrtl.const.uint<4>, !firrtl.const.bundle<c: uint<8>, d: uint<16>>) -> !firrtl.const.bundle<a: uint<4>, b: bundle<c: uint<8>, d: uint<16>>>
    // CHECK: %c0_ui1 = firrtl.constant 0 : !firrtl.const.uint<1>
    // CHECK: %reg = firrtl.regreset %clock, %c0_ui1, %9 : !firrtl.clock, !firrtl.const.uint<1>, !firrtl.const.bundle<a: uint<4>, b: bundle<c: uint<8>, d: uint<16>>>, !firrtl.bundle<a: uint<4>, b: bundle<c: uint<8>, d: uint<16>>>
  ...
  firrtl.module @TestBundleCreate_VB(out %out : !firrtl.vector<bundle<a: uint<8>, b: uint<16>>, 2>) {
    ...
    // CHECK: %0 = firrtl.bundlecreate %c1_ui8, %c2_ui16 : (!firrtl.const.uint<8>, !firrtl.const.uint<16>) -> !firrtl.const.bundle<a: uint<8>, b: uint<16>>
    // CHECK: %1 = firrtl.subfield %0[b] : !firrtl.const.bundle<a: uint<8>, b: uint<16>>
    // CHECK: %2 = firrtl.subfield %0[a] : !firrtl.const.bundle<a: uint<8>, b: uint<16>>
    %bundle1 = firrtl.bundlecreate %0, %1 : (!firrtl.const.uint<8>, !firrtl.const.uint<16>) -> !firrtl.const.bundle<a: uint<8>, b: uint<16>>

    // CHECK: %3 = firrtl.bundlecreate %c1_ui8, %c2_ui16 : (!firrtl.const.uint<8>, !firrtl.const.uint<16>) -> !firrtl.const.bundle<a: uint<8>, b: uint<16>>
    // CHECK: %4 = firrtl.subfield %3[b] : !firrtl.const.bundle<a: uint<8>, b: uint<16>>
    // CHECK: %5 = firrtl.subfield %3[a] : !firrtl.const.bundle<a: uint<8>, b: uint<16>>
    %bundle2 = firrtl.bundlecreate %0, %1 : (!firrtl.const.uint<8>, !firrtl.const.uint<16>) -> !firrtl.const.bundle<a: uint<8>, b: uint<16>>
    
    // CHECK: %6 = firrtl.vectorcreate %2, %5 : (!firrtl.const.uint<8>, !firrtl.const.uint<8>) -> !firrtl.const.vector<uint<8>, 2>
    // CHECK: %7 = firrtl.vectorcreate %1, %4 : (!firrtl.const.uint<16>, !firrtl.const.uint<16>) -> !firrtl.const.vector<uint<16>, 2>
    // CHECK: %8 = firrtl.bundlecreate %6, %7 : (!firrtl.const.vector<uint<8>, 2>, !firrtl.const.vector<uint<16>, 2>) -> !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<16>, 2>>
    ...
    // CHECK firrtl.connect %out, %8 : !firrtl.bundle<a: vector<uint<8>, 2>, b: vector<uint<16>, 2>>, !firrtl.const.bundle<a: vector<uint<8>, 2>, b: vector<uint<16>, 2>>
