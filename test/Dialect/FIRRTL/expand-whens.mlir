...
firrtl.module @shadow_connects(out %out : !firrtl.uint<1>) {
  ...
  firrtl.connect %out, %c1_ui1 : !firrtl.uint<1>, !firrtl.const.uint<1>
}
...
// CHECK-NEXT:   firrtl.connect %out, %c1_ui1 : !firrtl.uint<1>, !firrtl.const.uint<1>
// CHECK-NEXT: }
...
firrtl.module @shadow_when(in %p : !firrtl.uint<1>) {
  ...
  firrtl.when %p : !firrtl.uint<1> {
    ...
    firrtl.connect %w, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
...
// CHECK-NEXT:   %c1_ui2 = firrtl.constant 1 : !firrtl.const.uint<2>
// CHECK-NEXT:   %w = firrtl.wire  : !firrtl.uint<2>
// CHECK-NEXT:   firrtl.connect %w, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
// CHECK-NEXT: }
...
firrtl.module @set_in_both(in %clock : !firrtl.clock, in %p : !firrtl.uint<1>, out %out : !firrtl.uint<2>) {
  firrtl.when %p : !firrtl.uint<1> {
    %c0_ui2 = firrtl.constant 0 : !firrtl.const.uint<2>
    firrtl.connect %out, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  } else {
    %c1_ui2 = firrtl.constant 1 : !firrtl.const.uint<2>
    firrtl.connect %out, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
}
...
// CHECK-NEXT:   %c1_ui2 = firrtl.constant 1 : !firrtl.const.uint<2>
// CHECK-NEXT:   %1 = firrtl.mux(%p, %c0_ui2, %c1_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
...
firrtl.module @set_before_and_in_both(in %clock : !firrtl.clock, in %p : !firrtl.uint<1>, out %out : !firrtl.uint<2>) {
  ...
  firrtl.connect %out, %c2_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  firrtl.when %p : !firrtl.uint<1> {
    firrtl.connect %out, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  } else {
     firrtl.connect %out, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
...
// CHECK-NEXT:   %c0_ui2 = firrtl.constant 0 : !firrtl.const.uint<2>
// CHECK-NEXT:   %c1_ui2 = firrtl.constant 1 : !firrtl.const.uint<2>
// CHECK-NEXT:   %c2_ui2 = firrtl.constant 2 : !firrtl.const.uint<2>
// CHECK-NEXT:   %0 = firrtl.not %p : (!firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   %1 = firrtl.mux(%p, %c0_ui2, %c1_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
// CHECK-NEXT:   firrtl.connect %out, %1 : !firrtl.uint<2>, !firrtl.uint<2>
// CHECK-NEXT: }
...
// CHECK-NEXT:   %0 = firrtl.not %p : (!firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   %1 = firrtl.mux(%p, %c0_ui2, %c1_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
// CHECK-NEXT:   firrtl.connect %out, %c2_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
// CHECK-NEXT: }
...
// CHECK-NEXT:   %c0_ui2 = firrtl.constant 0 : !firrtl.const.uint<2>
// CHECK-NEXT:   %c1_ui2 = firrtl.constant 1 : !firrtl.const.uint<2>
// CHECK-NEXT:   %0 = firrtl.mux(%p, %c1_ui2, %c0_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
...
// CHECK-NEXT:   %c0_ui2 = firrtl.constant 0 : !firrtl.const.uint<2>
// CHECK-NEXT:   %c1_ui2 = firrtl.constant 1 : !firrtl.const.uint<2>
// CHECK-NEXT:   firrtl.connect %out, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
// CHECK-NEXT: }
...
firrtl.module @check_mux_return_type(in %p : !firrtl.uint<1>, out %out : !firrtl.uint<2>) {
  %c0_ui1 = firrtl.constant 0 : !firrtl.const.uint<1>
  %c1_ui2 = firrtl.constant 1 : !firrtl.const.uint<2>
  firrtl.connect %out, %c0_ui1 : !firrtl.uint<2>, !firrtl.const.uint<1>
  firrtl.when %p : !firrtl.uint<1> {
  } else {
    firrtl.connect %out, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
  // CHECK: firrtl.mux(%p, %c0_ui1, %c1_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<1>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
}
...
// CHECK-NEXT:   %c2_ui2 = firrtl.constant 2 : !firrtl.const.uint<2>
// CHECK-NEXT:   %0 = firrtl.and %p0, %p1 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK-NEXT:   %1 = firrtl.mux(%p1, %c1_ui2, %c0_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
// CHECK-NEXT:   %2 = firrtl.mux(%p0, %1, %c0_ui2) : (!firrtl.uint<1>, !firrtl.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
...
firrtl.module @nested2(in %clock : !firrtl.clock, in %p0 : !firrtl.uint<1>, in %p1 : !firrtl.uint<1>, out %out : !firrtl.uint<2>) {
  ...
  firrtl.when %p0 : !firrtl.uint<1> {
    ...
    firrtl.when %p1 : !firrtl.uint<1> {
      ...
      firrtl.connect %out, %c3_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
    }
  }
}
...
//CHECK-NEXT:   %c1_ui2 = firrtl.constant 1 : !firrtl.const.uint<2>
//CHECK-NEXT:   %c2_ui2 = firrtl.constant 2 : !firrtl.const.uint<2>
//CHECK-NEXT:   %c3_ui2 = firrtl.constant 3 : !firrtl.const.uint<2>
//CHECK-NEXT:   %0 = firrtl.and %p0, %p1 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
...
//CHECK-NEXT:   %3 = firrtl.mux(%p1, %c0_ui2, %c1_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
...
//CHECK-NEXT:   %8 = firrtl.mux(%p1, %c2_ui2, %c3_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
...
firrtl.module @InvalidValues(in %p: !firrtl.uint<1>, out %out0: !firrtl.uint<2>, out %out1: !firrtl.uint<2>, out %out2: !firrtl.uint<2>, out %out3: !firrtl.uint<2>, out %out4: !firrtl.uint<2>, out %out5: !firrtl.uint<2>) {
  ...
  firrtl.when %p : !firrtl.uint<1> {
    firrtl.connect %out0, %c2_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  } else  {
  ...
  firrtl.when %p : !firrtl.uint<1> {
    ...
    firrtl.connect %out1, %c2_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
  // CHECK: firrtl.connect %out1, %c2_ui2

  firrtl.connect %out2, %invalid_ui2 : !firrtl.uint<2>, !firrtl.uint<2>
  firrtl.when %p : !firrtl.uint<1> {
    firrtl.connect %out2, %c2_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
  ...
  firrtl.when %p : !firrtl.uint<1> {
    ...
    firrtl.connect %out3, %c2_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
  // CHECK: firrtl.connect %out3, %c2_ui2

  firrtl.connect %out4, %c2_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  firrtl.when %p : !firrtl.uint<1> {
    firrtl.connect %out4, %invalid_ui2 : !firrtl.uint<2>, !firrtl.uint<2>
  }
  // CHECK: firrtl.connect %out4, %c2_ui2

  firrtl.connect %out5, %c2_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  firrtl.when %p : !firrtl.uint<1> {
    firrtl.skip
  } else  {
...
firrtl.module @register_mux(in %p : !firrtl.uint<1>, in %clock: !firrtl.clock) {
  ...
  firrtl.connect %reg1, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  ...
  firrtl.when %p : !firrtl.uint<1> {
    firrtl.connect %reg2, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
  ...
  firrtl.when %p : !firrtl.uint<1> {
    firrtl.connect %reg3, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  } else {
    firrtl.connect %reg3, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
}
...
firrtl.module @bundle_types(in %p : !firrtl.uint<1>, in %clock: !firrtl.clock) {
  ...
  firrtl.when %p : !firrtl.uint<1> {
    ...
    firrtl.connect %w_a, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  } else {
    %w_a = firrtl.subfield %w[a] : !firrtl.bundle<a : uint<2>, b flip: uint<2>>
    firrtl.connect %w_a, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
  ...
  firrtl.connect %w_b0, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  firrtl.when %p : !firrtl.uint<1> {
  } else {
    %w_b1 = firrtl.subfield %w[b] : !firrtl.bundle<a : uint<2>, b flip: uint<2>>
    firrtl.connect %w_b1, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
}
...
firrtl.module @bundle_ports() {
  ...
  firrtl.connect %0, %c1_ui1 : !firrtl.uint<1>, !firrtl.const.uint<1>
}
...
firrtl.module @as_passive(in %p : !firrtl.uint<1>) {
  ...
  %c3_ui3 = firrtl.constant 3 : !firrtl.const.uint<3>
  %simple0_in = firrtl.instance test0 @simple2(in in : !firrtl.uint<3>)
  firrtl.connect %simple0_in, %c2_ui3 : !firrtl.uint<3>, !firrtl.const.uint<3>
  ...
  firrtl.when %p : !firrtl.uint<1> {
    ...
    firrtl.connect %simple1_in, %c3_ui3 : !firrtl.uint<3>, !firrtl.const.uint<3>
  }
  // CHECK: [[MUX:%.*]] = firrtl.mux(%p, %test0_in, %c3_ui3) : (!firrtl.uint<1>, !firrtl.uint<3>, !firrtl.const.uint<3>) -> !firrtl.uint<3>
...
firrtl.module @vector_simple(in %clock: !firrtl.clock, out %ret: !firrtl.vector<uint<1>, 1>) {
  ...
  // CHECK-NEXT: firrtl.connect %0, %c1_ui1 : !firrtl.uint<1>, !firrtl.const.uint<1>
}
...
firrtl.module @shadow_when_vector(in %p : !firrtl.uint<1>) {
  ...
  firrtl.when %p : !firrtl.uint<1> {
    ...
    firrtl.connect %0, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
    firrtl.connect %0, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
  // CHECK:      %w = firrtl.wire  : !firrtl.vector<uint<2>, 1>
  // CHECK-NEXT: %0 = firrtl.subindex %w[0] : !firrtl.vector<uint<2>, 1>
  // CHECK-NEXT: firrtl.connect %0, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
}
...
firrtl.module @multi_dim_vector(in %p : !firrtl.uint<1>) {
  ...
  firrtl.when %p : !firrtl.uint<1> {
    firrtl.connect %1, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
    firrtl.connect %2, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  } else {
    firrtl.connect %1, %c1_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
    firrtl.connect %2, %c0_ui2 : !firrtl.uint<2>, !firrtl.const.uint<2>
  }
  // CHECK:      [[MUX1:%.*]] = firrtl.mux(%p, %c0_ui2, %c1_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
  // CHECK-NEXT: firrtl.connect %1, [[MUX1]] : !firrtl.uint<2>, !firrtl.uint<2>
  // CHECK-NEXT: [[MUX2:%.*]] = firrtl.mux(%p, %c1_ui2, %c0_ui2) : (!firrtl.uint<1>, !firrtl.const.uint<2>, !firrtl.const.uint<2>) -> !firrtl.uint<2>
...
firrtl.module @vector_of_bundle(in %p : !firrtl.uint<1>, out %ret: !firrtl.vector<bundle<a:uint<1>>, 1>) {
  ...
  // CHECK-NOT: firrtl.connect %1, %c0_ui1 : !firrtl.uint<1>, !firrtl.const.uint<1>
  // CHECK:     firrtl.connect %1, %c1_ui1 : !firrtl.uint<1>, !firrtl.const.uint<1>
}
